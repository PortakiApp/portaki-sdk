...
2. **CI — Trusted Publishing** : pour **`@portaki/sdk`**, configurer sur npm : dépôt **`portaki-sdk`**, fichier **`publish-npm-sdk.yml`**. Pour les **`@portaki/module-*`** invités, configurer sur npm : dépôt **[portaki-modules](https://github.com/PortakiApp/portaki-modules)**, fichier **`publish-npm-packages.yml`** (voir la doc de ce dépôt).
...
4. **Version dans le dépôt** : bump **`version`** dans `packages/sdk/package.json` (SDK) **avant** publication — la CI **ne modifie pas** ce fichier. Les modules invités : bump dans **[portaki-modules](https://github.com/PortakiApp/portaki-modules)** (`modules/<id>/package.json`).
...
| [`publish-npm-sdk.yml`](../.github/workflows/publish-npm-sdk.yml) | Publie **`@portaki/sdk`** (`packages/sdk`) — manuel / release UI. |
...
Déclenché sur `push` / `pull_request` vers `main` et `develop` lorsque `packages/**`, `legacy/**`, `examples/**`, `pnpm-workspace.yaml`, `package.json` ou ce workflow changent.
...
**Version :** celle de **`packages/sdk/package.json`** (à faire évoluer dans une PR avant publication).
...
**Coordonnées :** `app.portaki:portaki-module-sdk` — version **release** dans **`legacy/java/pom.xml`** (sans **`-SNAPSHOT`**).
...
Les paquets **`@portaki/module-*`** et **`@portaki/sdk`** sont publiés en **0.3.2** sur npmjs (patch après des **0.3.0** avec dépendance `file:` incorrecte). **portaki-web** peut déclarer **`^0.3.2`** (ou **`^0.3.0`** pour rétrocompat semver) et exécuter **`pnpm install`**.
...
```bash
cd legacy/java && mvn install -DskipTests
```
