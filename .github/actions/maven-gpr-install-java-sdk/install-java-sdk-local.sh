...
SDK_POM="${SDK_DIR}/legacy/java/pom.xml"

if [[ ! -f "$SDK_POM" ]]; then
  echo "Expected portaki-sdk Java POM at legacy/java/pom.xml (resolved: ${SDK_POM})"
  exit 1
fi
...
if [[ -f "${SDK_DIR}/legacy/java-test-support/pom.xml" ]]; then
  echo "Installing app.portaki:portaki-module-sdk-test into local ~/.m2…"
  mvn -B install -f "${SDK_DIR}/legacy/java-test-support/pom.xml" -DskipTests
fi
