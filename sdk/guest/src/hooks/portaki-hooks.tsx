...
export function usePortakiContext(): PortakiContext {
...
export function usePortakiConfig<T extends Record<string, unknown>>(): T {
  const { config } = usePortakiContext()
  return config as T
}

export function usePortakiQuery<T>(
  queryName: string,
  ...
  const [error, setError] = useState<PortakiError | null>(null)
...
export function usePortakiCommand<TParams = Record<string, unknown>>(
  commandName: string,
): {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          try {
            ...
            msg = (errBody as { message?: string }).message ?? msg
          } catch {
            /* ignore */
          }
          throw new Error(msg)
...
export function usePortakiModuleQuery<T>(options: {
  moduleId: string
  queryName: string
  params?: Record<string, unknown>
  enabled?: boolean
