...
import { decodeModuleHmacKeyMaterial, type HmacPayload } from '../runtime/security/hmac.js'
...
export function verifyHmacToken(
  token: string | null,
  expected: { moduleId: string; queryName?: string; commandName?: string; stayId: string },
  keyMaterialB64: string,
  maxSkewMs: number = 120_000,
): boolean {
  ...
  try {
    ...
    payload = JSON.parse(json) as HmacPayload
  } catch {
    return false
  }
